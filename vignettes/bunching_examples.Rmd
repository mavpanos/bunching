...
This vignette is an introduction to the `bunching` package's basic toolkit, and shows examples of how it can be applied to data that feature bunching. The package implements the bunching estimator, which was originally developed to analyze responses to discrete changes in choice sets within public and labor economics, but can be applied to any setting that features such an optimization setting. The aim is to measure behavioral responses to changes in incentives, by estimating how much excess mass, in an otherwise smooth distribution, can be attributed to the responses in a discrete change in constraints at that same level.
...
- **`t0`** and **`t1`**: the marginal (average) tax rates below and above `z_star` in a kink (notch) setting
...
The rest of the inputs have set defaults. Among these, the important ones the user may want to experiment with are:

- **`binv`**: This is the bin version, which controls how the bins are grouped around `zstar`. The  default is "median", which places `zstar` in the median position in its bin. The other options are "min" and "max", which create bins with `zstar` being in the minimum or maximum position
- **`poly`**: The order of the polynomial used to fit the bin counts
- **`bins_excl_l`** and **`bins_excl_r`**: How many bins to the left and right of `zstar` to also include in the bunching (i.e. "excluded") region. This is particularly important when the density exhibits diffuse bunching around `zstar`
- **`extra_fe`**: Whether the counterfactual estimate should also control for a particular fixed point in the distribution that also features a bunching mass (or hole). This is useful when there is another focal point in the bandwidth that can affect the estimate of the bunching mass at `zstar` if not controlled for
- **`rn`**: Round numbers to also control for through fixed effects
- **`n_boot`**: How many bootstrapped samples to use to estimate (residual-based) standard errors
- **`correct`**: Whether to correct for the integration constraint
- **`correct_above_zu`**: This controls where to start shifting the counterfactual distribution up from when applying the integration constraint correction. If set to `TRUE`, it only shifts bins above $z_U$ (upper range of bunching region). Default is `FALSE`, which shifts all bins above $z^*$
- **`notch`**: Whether the analysis is for a notch or a kink. Default is `FALSE` (kink)
- **`force_notch`**: Whether to force the user choice of bins_excl_r. The default is `FALSE`, whereby the upper bound of the excluded region is estimated through an iterative process that equates the bunching and missing masses
- **`e_parametric`**: Whether to estimate elasticities using the parametric form or the reduced-form specifications. Default is `FALSE`
- **`e_parametric_lb`** and  **`e_parametric_ub`**: If both `notch = TRUE` and `e_parametric = TRUE` are chosen, the elasticity is found by a non-linear equation solver. `e_parametric_lb` and `e_parametric_ub`set the lower and upper bound of possible solution values for the elasticity
- **`seed`**: A value used as a seed for reproducability of standard errors
...
Using the package's helper functions `bin_data()` and `prep_data_for_fit()`, the chosen vector is binned into groups of binwidth $\delta$ around the bunching point `zstar` =  $z^*$. The following specification is then run:
...
$c_j$ is the observation count in bin $j$, $p$ is the order of polynomial used to fit the counts, and $z_L$ and $z_U$ stand for the lower and upper region that define the bunching region. The specification also allows the user to control for round number bunching, and for other fixed effects that feature a bunching mass in the estimation bandwidth outside the bunching range $z \in [z_L, z_U]$ but that are not associated with $z^*$ (through the $\rho$ and $\theta$ coefficient vectors respectively). 
...
$\hat{B}_0$ estimates the excess number of observations locating at $z^*$ due to the kink or notch. To be able to compare bunching mass across different kinks or notches that feature varying heights of counterfactuals, we use a normalization where we divide the total excess mass with the height of the counterfactual at $z^*$. This returns the \textit{normalized excess mass}, which is a central parameter of interest besides elasticity estimates:
...
As expected, the counterfactual to the right of $z^*$ is now lower. The effect of this is to pull the height of the counterfactual at $z^*$ slightly lower than before, leading to a larger estimate for the normalized excess mass of `r kink1_no_corr$b` compared to `r kink1_param$b`.
...
```
This version now creates some diffuse bunching by shifting some to the first bin to the right of $z^*$, so `bins_excl_r` should be set to 1. We can also increase the flexibility of the polynomial by setting `poly = 9`. Finally, let's get an estimate of the elasticity using the parametric formulation, using `e_parametric = TRUE`.

```{r}
kink5 <- bunchit(z_vector = bunching_data$kink, zstar = 10000, binv = "max", 
                 binwidth = 100, bins_l = 20, bins_r = 20, bins_excl_r = 1,
                 poly  = 6, t0 = 0, t1 = .2, e_parametric = TRUE, p_b = TRUE,
                 seed = 1, p_title = "Kink with diffuse bunching and zstar max in bin")
kink5$plot
```
...
If you require further flexibility than what is provided, you can instead build the whole plot from scratch. From the list of results returned from `bunchit()`, simply use `cf_density` for the estimated counterfactual, and  columns `bin` and `freq_orig` from the `data` dataframe for the bins and per-bin count.
